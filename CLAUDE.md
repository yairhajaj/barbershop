...
**מטרה:**
- שלב א (עכשיו): לפתח עם HAJAJ כמקרה בוחן — **אין עדיין לקוחות אמיתיים**, בפיתוח פעיל
- שלב ב (עתיד): SaaS — למכור לספרות/עסקים אחרים בישראל
- **חזון:** להיות האפליקציה הטובה והנוחה ביותר לניהול מספרות בישראל

**ארכיטקטורה:** Single-tenant כרגע. כשמרחיבים ל-SaaS → להוסיף `business_id` לכל הטבלאות.
