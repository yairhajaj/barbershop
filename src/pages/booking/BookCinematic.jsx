 ...
 * Step 0: Staff → Step 1: Service → Step 2: Date + Time → /book/confirm
...
const pageVariants = {
  ...
  center: { x: 0, opacity: 1, filter: 'blur(0px)', transition: { duration: 0.48, ease: [0.22, 1, 0.36, 1] } },
  exit:  (dir) => ({ x: dir > 0 ? '-55%' : '55%', opacity: 0, filter: 'blur(6px)', transition: { duration: 0.28, ease: 'easeIn' } }),
}
...
export default function BookCinematic() {
  ...
  // Pre-select staff from URL param (?staff=ID) — used by StaffProfile CTA
  useEffect(() => {
  ...
  // Fetch blocked times when staff or date changes
  useEffect(() => {
  ...
  // Generate available slots — same logic as BookAll.jsx
  useEffect(() => {
  ...
  // Build date options (exclude closed days + Shabbat)
  const shabbatConfig = {
  ...
  // Filter services eligible for selected staff
  ...
  // CTA unlocks when a selection is made on the current step
  const ctaEnabled =
    step === 0 ? selStaff !== undefined :
    step === 1 ? !!selService :
    !!selSlot
  ...
  return (
    <div dir="rtl" style={{ minHeight: '100dvh', position: 'relative', background: '#0d0a07' }}>

      {/* ── Ambient blurred staff photo background ── */}
      <AnimatePresence>
        {bgPhoto && (
          <motion.div
            key={bgPhoto}
            ...
            style={{
              ...
              filter: 'blur(55px) saturate(0.45)', transform: 'scale(1.18)',
              opacity: 0.16,
            }}
          />
        )}
      </AnimatePresence>
      {/* dark overlay always on top of bg photo */}
      <div style={{ position: 'fixed', inset: 0, zIndex: 1, pointerEvents: 'none', background: 'linear-gradient(160deg, rgba(13,10,7,0.88) 0%, rgba(13,10,7,0.78) 100%)' }} />

      {/* ── Main content ── */}
      <div style={{ position: 'relative', zIndex: 2, minHeight: '100dvh', paddingTop: 68, paddingBottom: 130 }}>
        ...
        <AnimatePresence custom={dir} mode="wait">
          {step === 0 && (
            <motion.div key="staff" custom={dir} variants={pageVariants} initial="enter" animate="center" exit="exit">
              <StepStaff staff={staff} loading={staffLoading} selected={selStaff} onSelect={m => { setSelStaff(m); setSelSlot(null) }} />
            </motion.div>
          )}
          {step === 1 && (
            <motion.div key="service" custom={dir} variants={pageVariants} initial="enter" animate="center" exit="exit">
              <StepService services={eligibleServices} selected={selService} onSelect={s => { setSelService(s); setSelSlot(null) }} />
            </motion.div>
          )}
          {step === 2 && (
      ...
      {/* ── Sticky CTA (spring slides up when a selection is made) ── */}
      <AnimatePresence>
        {ctaEnabled && (
          <motion.div
          ...
          >
            <motion.button
              whileTap={{ scale: 0.97 }}
              onClick={step < 2 ? goForward : goConfirm}
              style={{
                width: '100%', padding: '16px 0', borderRadius: 16, border: 'none',
                background: 'linear-gradient(135deg, #d4af37 0%, #b8860b 100%)',
                color: '#fff', fontSize: 16, fontWeight: 800, cursor: 'pointer',
                letterSpacing: '0.02em', boxShadow: '0 6px 28px rgba(212,175,55,0.38)',
              }}
            >
              {step < 2 ? 'המשך ←' : 'לאישור הזמנה ←'}
            </motion.button>
          </motion.div>
        )}
      </AnimatePresence>
...
/* ─────────────────────────────────────────────────
   Step bar — dots progress indicator + back button
───────────────────────────────────────────────── */
      ...
      <div style={{ display: 'flex', gap: 7, alignItems: 'center' }}>
        {[0, 1, 2].map(i => (
          <motion.div
            ...
            animate={{
              width:      i === step ? 26 : 7,
              background: i === step ? '#d4af37' : i < step ? 'rgba(212,175,55,0.45)' : 'rgba(255,255,255,0.18)',
            }}
...
function StepStaff({ staff, loading, selected, onSelect }) {
  return (
    <div style={{ padding: '0 20px' }}>
      <StepHeading stepNum={1} title="מי ייטפל בך?" />
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          >
            ...
            <div style={{ flex: 1, textAlign: 'right' }}>
              <p style={cardNameStyle}>כל ספר פנוי</p>
              <p style={cardSubStyle}>השעות המוקדמות ביותר</p>
            </div>
          ...
          {staff.map(member => (
            <motion.button
              ...
              style={cardStyle(selected?.id === member.id)}
            >
              <div style={{ width: 56, height: 56, borderRadius: 14, overflow: 'hidden', flexShrink: 0, background: '#2a1f0e' }}>
                {member.photo_url
                  ...
                  : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <span style={{ fontSize: 22, color: 'rgba(255,255,255,0.35)' }}>{member.name[0]}</span>
                    </div>
                }
              </div>
              <div style={{ flex: 1, textAlign: 'right' }}>
                <p style={cardNameStyle}>{member.name}</p>
                {member.bio && <p style={{ ...cardSubStyle, WebkitLineClamp: 1, display: '-webkit-box', WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{member.bio}</p>}
              </div>
              {selected?.id === member.id && <CheckMark />}
            </motion.button>
          ))}
        </motion.div>
      )}
...
function StepService({ services, selected, onSelect }) {
  return (
    <div style={{ padding: '0 20px' }}>
      <StepHeading stepNum={2} title="איזה שירות?" />
      ...
      <motion.div variants={stagger} initial="hidden" animate="show" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {services.map(svc => (
          <motion.button
            ...
            style={cardStyle(selected?.id === svc.id)}
          >
            <div style={{ ...avatarStyle(false), fontSize: 22, background: 'rgba(255,255,255,0.06)' }}>
              {serviceIcon(svc.name)}
            </div>
            <div style={{ flex: 1, textAlign: 'right' }}>
              <p style={cardNameStyle}>{svc.name}</p>
              <p style={cardSubStyle}>{svc.duration_minutes} דקות</p>
            </div>
            <div style={{ textAlign: 'left', flexShrink: 0 }}>
              <p style={{ color: 'var(--color-gold)', fontWeight: 800, fontSize: 16, margin: 0 }}>
                {svc.price ? `₪${svc.price}` : ''}
              </p>
            </div>
  ...
  return (
    <div>
      <div style={{ padding: '0 20px 20px' }}>
        <StepHeading stepNum={3} title="מתי?" />
      </div>
        ...
        {dateOptions.map(d => {
          ...
          return (
            <motion.button
              ...
              onClick={() => onDate(d)}
              style={{
                flexShrink: 0, width: 54, padding: '10px 0', borderRadius: 14,
                display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2,
                background: active ? '#d4af37' : 'rgba(255,255,255,0.06)',
                border: `1.5px solid ${active ? '#d4af37' : 'rgba(255,255,255,0.1)'}`,
                cursor: 'pointer', transition: 'all 0.2s',
              }}
            >
              <span style={{ fontSize: 10, fontWeight: 600, color: active ? '#fff' : 'rgba(255,255,255,0.45)' }}>
                {HE_DAY[d.getDay()]}
              </span>
              <span style={{ fontSize: 22, fontWeight: 800, lineHeight: 1.1, color: '#fff' }}>
                {d.getDate()}
              </span>
              <span style={{ fontSize: 10, color: active ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.3)' }}>
                {HE_MON[d.getMonth()]}
              </span>
            </motion.button>
          )
        ...
        <AnimatePresence mode="wait">
          {slotsLoading ? (
            ...
            <motion.p key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              style={{ color: 'rgba(255,255,255,0.35)', textAlign: 'center', padding: '48px 0', fontSize: 14 }}>
            ...
            >
              {slots.map((slot, i) => {
                ...
                return (
                  <motion.button
                    key={i}
                    variants={{ hidden: { opacity: 0, scale: 0.82 }, show: { opacity: 1, scale: 1, transition: { duration: 0.3, ease: 'easeOut' } } }}
                    whileTap={{ scale: 0.93 }}
                    onClick={() => onSlot(slot)}
                    style={{
...
function StepHeading({ stepNum, title }) {
  return (
    ...
    >
      <p style={{ color: 'var(--color-gold)', fontSize: 11, fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase', marginBottom: 7, opacity: 0.85 }}>
        שלב {stepNum} מתוך 3
      </p>
      <h2 style={{ color: '#fff', fontSize: 30, fontWeight: 800, margin: 0, lineHeight: 1.2 }}>{title}</h2>
      <motion.div
        initial={{ width: 0 }}
        animate={{ width: 36, transition: { delay: 0.2, duration: 0.5 } }}
        style={{ height: 2, background: 'var(--color-gold)', borderRadius: 2, marginTop: 10, opacity: 0.7 }}
      />
    </motion.div>
  )
}
...
function CheckMark() {
  return (
    <motion.div
      ...
      animate={{ scale: 1, rotate: 0, transition: { type: 'spring', stiffness: 420, damping: 20 } }}
      style={{
...
function LoadingDots() {
  return (
    <div style={{ display: 'flex', gap: 8, justifyContent: 'center', padding: '48px 0' }}>
      {[0, 1, 2].map(i => (
        <motion.div
          key={i}
          animate={{ y: [0, -10, 0] }}
...
const avatarStyle = (_unused) => ({
  width: 48, height: 48, borderRadius: 14, flexShrink: 0,
  background: 'rgba(212,175,55,0.12)',
  display: 'flex', alignItems: 'center', justifyContent: 'center',
})

const cardNameStyle = { color: '#fff', fontWeight: 700, fontSize: 15, margin: 0 }
const cardSubStyle  = { color: 'rgba(255,255,255,0.45)', fontSize: 12, margin: '3px 0 0' }
