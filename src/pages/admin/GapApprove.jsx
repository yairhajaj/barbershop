...
export function GapApprove() {
  ...
  const [done, setDone] = useState(null) // 'approved' | 'cancelled'
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select('*, appointments(id, start_at, end_at, services(name), staff(name), profiles(name, phone, push_token))')
      .eq('token', token)
      .single()
      .then(({ data, error }) => {
  ...
  async function handleApprove() {
    setPhase('processing')
    try {
      // Reset expiry and change status to pending so customer can respond
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-3 text-center px-4">
        ...
        <p className="font-bold text-lg" style={{ color: 'var(--color-text)' }}>
          {done === 'approved' ? 'ההצעה נשלחה ללקוח!' : 'ההצעה בוטלה'}
        </p>
        <button
          onClick={() => navigate('/admin/appointments')}
          className="px-4 py-2 rounded-xl text-sm font-bold text-white mt-2"
