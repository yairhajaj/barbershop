...
export function Appointments() {
  ...
  async function sendApprovalRequest(candidate, cancelledAppt) {
    ...
    if (tokens.length > 0) {
      await supabase.functions.invoke('send-push', {
        body: {
          title: `⚡ חור ביומן ב-${formatTime(new Date(cancelledAppt.start_at))}`,
          body: 'האם להפעיל סגירת חור? קודם רשימת המתנה, ואחר כך הזזת תורים',
          tokens,
          url: approveUrl,
        },
      })
  ...
  function shareInvoiceWhatsApp(appt, inv, phone) {
    ...
    const msg = encodeURIComponent(
      ...
      `${docLabel(settings?.business_type)} מס׳ ${inv?.invoice_number} עבור ${appt.services?.name}.\n` +
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100svh - 56px - 64px)', overflow: 'hidden' }}>
      ...
      <div className="flex flex-col gap-1 px-1 pb-1.5 flex-shrink-0" style={{ minHeight: 90 }}>
        ...
        {/* ── Row 1: Navigation + Date label + Book button ── */}
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="h-9 px-3 rounded-xl flex items-center gap-1.5 flex-shrink-0 transition-all btn-primary text-xs font-bold"
            title="קבע תור"
          >
        ...
        {/* ── Row 2: Staff filter + View switcher + Block + Settings ── */}
        <div className="flex items-center gap-1.5">

          {/* Staff filter */}
          <select
            className="flex-1 min-w-0 h-9 rounded-xl border text-xs font-medium px-2"
          ...
          {/* View switcher — labels always visible */}
          <div
          ...
          <button
            ...
            className="h-9 px-2 rounded-xl border flex items-center gap-1 flex-shrink-0 transition-colors text-xs font-semibold"
            style={{ background: 'var(--color-card)', borderColor: 'var(--color-border)', color: 'var(--color-text)' }}
            title="חסימת שעות"
          >
            ...
            {invoicingEnabled && (() => {
              ...
              return (
                <div className="space-y-3">
                  ...
                  <p className="text-xs font-semibold mb-2" style={{ color: 'var(--color-muted)' }}>בחר אמצעי תשלום להפקת {docLabel(settings?.business_type)}:</p>
